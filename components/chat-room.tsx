    ...
    const pollUpdates = async () => {
      ...
      try {
        const response = await fetch(
          `/api/chat/${roomId}?lastMessageId=${messages[messages.length - 1]?._id || ""}`
        );
        ...
        if (response.ok) {
            ...
            setMessages((prev) => {
              const updatedMessages = prev.map((msg) => {
                if (msg._id.startsWith("temp-")) {
                  const realMessage = data.messages.find(
                    (newMsg: MessageData) =>
              ...
              const newMessages = data.messages.filter(
                (newMsg: MessageData) =>
                  !updatedMessages.some((msg) => msg._id === newMsg._id)
              );
          ...
          if (data.typingUsers) {
            setTypingUsers(
              data.typingUsers.filter((user: TypingUser) => user.userId !== userId)
            );
          }
        }
      } catch (error) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (result.success && result.message && typeof result.message !== "string") {
        const messageData = result.message as MessageData; // Type assertion setelah pengecekan
        setMessages((prev) =>
          prev.map((msg) =>
            msg._id === tempId
        ...
        toast({
          ...
          description: typeof result.message === "string" ? result.message : "Please try again",
          variant: "destructive",
        });
      }
    } catch (error) {
